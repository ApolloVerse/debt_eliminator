...
export const Incomes = () => {
  ...
  useEffect(() => {
    ...
    if (!user) return;
    const q = query(collection(db, 'incomes'), where('userId', '==', user.uid), where('isActive', '==', true));
    return onSnapshot(q, (s) => setIncomes(s.docs.map(d => ({ id: d.id, ...d.data() } as Income))));
  }, []);
  ...
  const onSubmit = async (data: Partial<Income>) => {
    ...
    try {
      ...
      if (editingIncome) {
        ...
        await addDoc(collection(db, 'incomes'), { ...payload, createdAt: serverTimestamp() });
  ...
  const deleteIncome = async (id: string) => {
    ...
    await updateDoc(doc(db, 'incomes', id), { isActive: false });
