...
export const Expenses = () => {
  ...
  const openAdd = () => {
    ...
    reset({ name: '', amount: 0, category: 'other', recurrenceDate: '', recurrenceDay: 1 });
    setIsModalOpen(true);
  };
  ...
  const openEdit = (expense: Expense) => {
    ...
    setValue('recurrenceDate', expense.recurrenceDate || '');
    setValue('recurrenceDay', expense.recurrenceDay || 1);
    setIsModalOpen(true);
  };
  ...
  const onSubmit = async (data: Partial<Expense>) => {
    ...
    try {
      ...
      if (editingExpense) {
        await supabase.from('expenses').update(payload).eq('id', editingExpense.id).throwOnError();
        toast.success('Despesa atualizada');
      } else {
        await supabase.from('expenses').insert([{ ...payload, createdAt: new Date().toISOString() }]).throwOnError();
  ...
  const deleteExpense = async (id: string) => {
    ...
    try {
      await supabase.from('expenses').update({ isActive: false }).eq('id', id);
